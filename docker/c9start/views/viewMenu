...
#└──────────────────────────────────────────────────┘
    ...
    # Проходим по пунктам меню
        case $? in
            1) controller:Menu 'gitclone' ;; # Выбран пункт: "Git clone"
...
#└────────────────────────────────────────────────┘
    ...
    # Проходим по пунктам меню
        case $? in
            1) controller:Menu 'gitclone' ;; # Выбран пункт: "Обновить"
            0)       menu:Exit            ;; # Выбран пункт: "Exit"
        esac
    ;;
...
#┌─────────────────────────────────────────────────────┐
#│ Предлагает пользователю только остановить контейнер │
#└─────────────────────────────────────────────────────┘
    'stop')
    # Выводим меню на экран
        menu "$HEADER" 1
        #   Цвет      Строка
            Green "1. Stop       Остановить '$RUN_IMAGE:$RUN_VERSION'"
            Green '2. Настройки  Перейти в раздел'
             Exit '0. Exit'
             
    # Проходим по пунктам меню
        case $? in
            1) controller:Menu 'stop'          ;; # Выбран пункт: "Stop"
            2)       view:Menu 'settings' "$1" ;; # Выбран пункт: "Настройки"
            0)       menu:Exit                 ;; # Выбран пункт: "Exit"
        esac
    ;;
...
#└────────────────────────────────────────────────┘
    ...
    # Проходим по пунктам меню
        case $? in
            1) view:Menu 'input_ssh' "$1" ;; # Выбран пункт: "Создать"
...
#└─────────────────────────────────────────────────────────┘
    ...
    # Предлагаем указать имя файла
        input "$GIT_USER" 'controller:Menu is_valid_ssh'
