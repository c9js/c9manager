...
#│ предлагаем пользователю ввести её вручную       │
#└─────────────────────────────────────────────────┘
    ...
    # Предлагаем пользователю ввести команду вручную
...
#│ предлагаем пользователю ввести его вручную         │
#└────────────────────────────────────────────────────┘
    ...
    # Выводим информацию со списком образов
        ...
        echo 'c9start   Образ для первого старта'
        echo '---------------------------------------------'
        
    # Предлагаем пользователю ввести имя образа вручную
...
# Контейнеры не найдены
    if ! docker:isContainer "name=^${IMAGE_RUN}$"; then
...
#└──────────────────────────────────────────┘
    ...
    # Старый образ не найден
        if ! docker:isImage "$IMAGE_RUN"; then
        # Выводим финальное сообщение и завершаем процесс
...
#└───────────────────────────────────────────┘
    ...
    # Получаем ID-образа
        ...
        # прямо в условии в if-е, вот тут [ ! docker:isImage "$IMAGE_RUN" ]
    fi
...
#└─────────────────────────────────────────┘
    ...
    # Старый образ не найден
        if [[ "$COMMAND" == 'build' ]] || ! docker:isImage "$IMAGE_RUN"; then
