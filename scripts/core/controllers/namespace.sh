...
menu:Last()    { core:Menu 'menu:Last'    "$@"; } # Переходит в текущий раздел
...
#│ Docker (докер) │
#└────────────────┘
docker:is_image()      { core:Docker 'is_image'      "$@"; } # Проверяет существуют-ли образы
docker:is_container()  { core:Docker 'is_container'  "$@"; } # Проверяет существуют-ли контейнеры
