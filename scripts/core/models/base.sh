...
#└────────────────────────────────────┘
    'IF')
    # Список аргументов
        local arg1="$2"     # Первой аргумент
        local operator="$3" # Оператор
        local arg2="$4"     # Второй аргумент
        local msg1          # Первой сообщение
        local msg2          # Второй сообщение
        local a=$#          # Порядковый номер
        local b             # Порядковый номер
        local c             # Порядковый номер
        
    # Получаем порядковый номер
        let b=$#-1
        let c=$#-2
        
    # Сохраняем первое ответвление
        if [ "${!b}" != ':' ]; then
            msg1="${!a}"
        fi
        
    # Сохраняем второе ответвление
        if [ "${!b}" == ':' ]; then
            msg1="${!c}"
            msg2="${!a}"
        fi
        
    # Проверяем первое ответвление
        if [ "$arg1" "$operator" "$arg2" ]; then
            echo "$msg1"
            return
        fi
        
    # Проверяем второе ответвление
        [ "${!b}" == ':' ] && echo "$msg2"
    ;;
