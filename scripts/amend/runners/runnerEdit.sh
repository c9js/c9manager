...
#█                     █
#▀─────────────────────▀
typeset -a REFS_ALL     # Список всех ссылок (кроме текущей ветки)
typeset -A COMMITS_ALL  # Список всех коммитов
typeset -a COMMITS_EDIT # Список коммитов которые нужно пересоздать
...
#└──────────────────────────┘
    'is_status')
    # Получаем статус
        RUN git status --porcelain && return 1
