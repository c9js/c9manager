...
declare -A menuLink     # Список ссылок для возврата в предыдущий раздел
...
# Сохраняем новое состояние выбранного пункта меню
    [ -n "$menuID" ] && menuState[$menuID]=$REPLY
...
#└─────────────────────────────┘
menu:Back() {
    view:Menu "${menuLink[$menuCurrent]}" "$@"
}
